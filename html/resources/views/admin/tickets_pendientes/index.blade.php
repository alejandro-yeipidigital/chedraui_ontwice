...
@section('content')
    <div class="be-content">
        <div class="page-head">
            <h2 class="page-head-title">Tickets Pendientes</h2>
        ...
        @if(Session::has('validation_message'))
            <div class="col col-8 offset-2" align="center">
                <p class="alert alert-info">{{ Session::get('validation_message') }}</p>
            </div>
        @endif
                                ...
                                <tbody>
                                    @foreach($tickets as $ticket)
                                        <tr>
                                            <td>{{ $country }}</td>
                                            <td>{{ $promotion }}</td>
                                            <td>{{ $ticket->temporality_name }}</td>
                                            <td><a href="{{ route('admin.users.show', $ticket->user_id) }}">{{ $ticket->name . ' ' . $ticket->middle_name . ' ' . $ticket->last_name }}</a></td>
                                            <td>{{ $ticket->created_at }}</td>
                                            <td>
                                                @if($ticket->validated == 1 && $ticket->valid == 1)
                                                    <span class="text-success">Valido</span> <i class="mdi mdi-check-circle text-success" style="font-size: 18px;"></i>
                                                @elseif($ticket->validated == 1 && $ticket->valid == 0)
